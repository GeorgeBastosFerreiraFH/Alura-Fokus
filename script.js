...
const musica = new Audio('/sons/luna-rise-part-one.mp3');                      //poderia usar direto readFile('')
musica.loop = true;
...
const contagemRegressiva = () => {
    if (tempoDecorridoEmSegundos <= 0) {
        ...
        if (focoAtivo, curtoAtivo) {
            const evento = new CustomEvent('FocoFinalizado');
            const evento2 = new CustomEvent('CurtoFinalizado');
            const evento3 = new CustomEvent('LongoFinalizado');
            document.dispatchEvent(evento);
            document.dispatchEvent(evento2);
            document.dispatchEvent(evento3);
        }
        zerar();
        return;
    }
